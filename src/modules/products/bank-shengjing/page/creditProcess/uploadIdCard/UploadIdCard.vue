 ...
 * @LastEditors: hsf
 * @LastEditTime: 2024-12-01 10:20:14
 * @Description:
-->
<template>
  <div class="authentication beijing-common">
    ...
    <div class="user-information">
      <van-form validate-first :label-width="70" ref="formDynamic">
        ...
        <van-field v-model="idCard.validDate" name="validDate" disabled label="有效日期" placeholder="扫描识别后自动填充" />
      </van-form>
    </div>
...
import { idCardOCR, idCardOCRSJ } from '@/request/interface/txQrc/readerPci';
...
import {
  jdugeNewUserRoute,
  updateBeiJingUserInfo,
  updateUserSaleInfo,
  updateUserSaleInfoNew
...
import { beijingChangePhone, sendSmsCode } from '@/request/interface/impower/UserInfo';
...
export default {
  ...
  computed: {
    isSubmitDisabledUpdateInfo() {
      return this.idCard.idNum && this.phoneInfo.smsCode;
    }
  },
  ...
  methods: {
    async init() {
      ...
      let { referrer, productId, applyUnionKey, phone } = await pageKeyInfoSearch.call(this);
      referrer = '252';
      if (referrer) {
      ...
      if (this.picFace && this.picNationalEm) {
        try {
          ...
          if (params.frontImageUrl && params.backImageUrl) {
            let ocrRes = await idCardOCRSJ(params);
    ...
    getProtocolText(protocol) {
      let localProtocolList = [
        'https://prod-xm-1256921822.cos.ap-shanghai.myqcloud.com/saas//contract_template/sj/个人信息查询和使用授权书.pdf',
        'https://prod-xm-1256921822.cos.ap-shanghai.myqcloud.com/saas//contract_template/sj/借款协议.pdf',
        'https://prod-xm-1256921822.cos.ap-shanghai.myqcloud.com/saas//contract_template/sj/征信查询授权书.pdf',
        'https://prod-xm-1256921822.cos.ap-shanghai.myqcloud.com/saas//contract_template/sj/委托担保合同.pdf'
      ];

      this.protocolText = localProtocolList[protocol.index];
    },
      ...
      this.$refs.formDynamicSms.validate(['phoneNum']).then(async (res) => {
        if (!res) {
          ...
          try {
            ...
            let sendRes = await sendSmsCode(this.phoneInfo.phoneNum);
          ...
          if (!res) {
            this.$refs.formDynamicSms.validate(['phoneNum', 'smsCode']).then(async (res) => {
              if (!res) {
                ...
                idCard.productId = this.productId || '9999';
                ...
                idCard.referenceNo = '123456';

                try {
                  this.$toasting.show({ message: '上传中', forbidClick: true });

                  // 1、update idCard info
                  const updateInfoRes = await updateUserSaleInfoNew(idCard);

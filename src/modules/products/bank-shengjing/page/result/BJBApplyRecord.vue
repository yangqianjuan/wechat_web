  ...
  methods: {
    async init() {
      try {
        ...
        let unionKeyRes = await getUnionKey({
          referrer: this.referrer,
          productId: '1155'
        });

        if (unionKeyRes?.data?.success && unionKeyRes.data.data) {
          this.applyUnionKey = unionKeyRes.data.data;
        }

        let applyListRes = await getPersonApplyList();

        if (applyListRes?.data?.success && applyListRes.data.data) {
          this.applyList = applyListRes.data.data;
        }
      } catch (error) {
        this.$toasting.error({ msg: error?.data?.msg || '获取申请记录失败！' });
      } finally {
        this.$toasting.hide();
    ...
    gotoDetail(index) {
      ...
      // 暂时只是允许 -- '1155' 产品的跳转
      if (String(apply?.productId) === '1155') {
        this.gotoApply();
      }
    },
    ...
    async gotoApply() {
      try {
        let nextPath = await queryNextByUnionKeyAndProdId('1155', this.applyUnionKey);
        this.$router.replace({
          path: nextPath,
          query: { referrer: this.referrer, productId: '1155', applyUnionKey: this.applyUnionKey }
        });
      } catch (e) {

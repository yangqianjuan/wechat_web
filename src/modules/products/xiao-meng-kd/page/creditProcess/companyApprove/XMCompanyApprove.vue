  ...
  <div class="company-approve-wrapper">
    ...
    <div v-if="!isStepUpload">
      <Form :model="busindessLicense" :label-width="150" :rules="ruleValidate" ref="formDynamic" label-position="left">
        ...
        <!-- <FormItem label="月均营业额（万元）" prop="monthlyTurnover">
          <i-input v-model="busindessLicense.monthlyTurnover" type="number"></i-input>
        </FormItem> -->
      </Form>
      <div class="notice">
        <!-- <div class="notice-one">尊敬的客户：您的月均营业额将会影响授信审批，</div> -->
        <div>请确认企业信息无误后，再提交！</div>
      </div>
    </div>
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        // monthlyTurnover: [{ required: true, message: '请填写月均营业额!', trigger: 'blur' }]
      },
      protocolText: [],
  ...
  methods: {
      ...
      if (this.picFace) {
        ...
        this.uploadUtil(picFace)
          ...
          .then(res => {
            ...
            // busindessLicense.monthlyTurnover = '';
    ...
    submitInfo(name) {
      this.$refs[name].validate(valid => {
        ...
        // let monthlyTurnover = parseFloat(busindessLicense.monthlyTurnover);
        // if (typeof monthlyTurnover !== 'number') {
        //   this.$notify.info({
        //     msg: '您输入的数据有误！'
        //   });
        //   return;
        ...
        this.$store.commit('setProductId', '1000');
        this.$store.commit('setReferrer', referrer);
        busindessLicense.referrer = referrer;
        busindessLicense.productId = '1000';
        this.isLoading = true;
        let params = {
          ...busindessLicense
          // monthlyTurnover: monthlyTurnover * 10000
        };

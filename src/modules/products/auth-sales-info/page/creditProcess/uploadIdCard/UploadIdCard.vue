...
import { salesIdCardOCR } from '@/request/interface/txQrc/readerPci';
import initUploadObj from '@/controller/upload';
import { dataURLtoFile, jdugeImgDirection, mergeObj } from '@/utils/common/common';
import { jdugeNewUserRoute, updateUserSaleInfo } from '@/controller/userController';
...
export default {
  ...
  methods: {
      ...
      try {
        ...
        if (this.wxjs_environment == 'miniprogram') {
          ...
          if (result?.data?.success && result?.data?.data) {
            if (result.data.data.threeMainStatus === '01' && result.data.data.faceIdStatus === '01') {
              setTimeout(() => {
                this.$router.replace({
        ...
        this.getProtocolList('sales');
        // getUnionKey({ referrer: this.referrer, productId: this.productId })
        //   .then(res => {
        //     if (res.data.success && res.data.data) {
        //       let time = 1000 * 60 * 60 * 24 * 7;
        //       const applyUnionKey = res.data.data;
     ...
     *      2、验证身份证信息salesIdCardOCR，验证通过到信息确认部分
     */
    uploadIdPic() {

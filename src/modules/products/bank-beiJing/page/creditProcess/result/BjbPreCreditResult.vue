<template>
  <div class="loans-result-wrapper beijing-common">
    ...
    <!-- 职业信息出错的提示文本 -->
    <div class="warning-text job-fail" v-if="applyInfo.applyStatus === 'JOB_FAIL'">
      尊敬的客户：您的职业信息校验不通过，请登录【广发银行-手机银行】完成修改！若已完成修改，请点击下方完成修改按钮，重新发起授信！
    </div>
  ...
  methods: {
    init() {
      ...
      if (this.$route.query.applyInfo) {
        ...
        this.warnText = WARN_TEXT_BYPROD[this.applyInfo.productId || '1155'];
        this.jdugeApplyDesc();
      } else {
        // 用户通过外部链接，回调的形式进入的

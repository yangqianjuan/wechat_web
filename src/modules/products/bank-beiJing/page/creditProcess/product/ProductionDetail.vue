  ...
  methods: {
    init() {
      try {
        ...
        const prid = this.getUrlParam('prid') || '1155';
        ...
        Promise.all([this.jdugeNewUser(), this.getProtocolList(this.payload.productId), this.getProduct()]);
      } catch (error) {
        console.log(error);
      }
    },
    ...
    async getProduct() {
      let productRes = await getProduceDetail({ productId: '1155' });
    ...
    async succHandle() {
      try {
        ...
        const nextPath = await queryNextByUnionKeyAndProdId(this.payload.productId);

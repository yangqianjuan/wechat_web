 ...
 * @LastEditors: hsf
 * @LastEditTime: 2023-07-31 14:19:30
 * @Description:
-->
<template>
...
export default {
  ...
  beforeDestroy() {
    this.clearStatusRead('/bjb_confirm_loan', '1155', this.formateProtocolRaw);
  },
  ...
  methods: {
    ...
    async getCreditNum() {
      try {
        ...
        if (canUseCreditInfo) {
          let operateApplyRecord = await getBeijingSingleBorrowInfoRecord({
            applyUnionKey: canUseCreditInfo.applyUnionKey
          });
          // update unionKey
          sessionStorage.setItem('union_key', canUseCreditInfo.applyUnionKey);
          sessionStorage.setItem('union_key_beijing', canUseCreditInfo.applyUnionKey);
          this.applyUnionKey = canUseCreditInfo.applyUnionKey;

          if (operateApplyRecord?.data?.success && operateApplyRecord.data.data) {
            // add check  0: pass;    1: lose efficacy;
            if (operateApplyRecord?.data?.data.code === 1) {
              setTimeout(() => {
                this.$router.replace({
                  ...
                  query: {
                    ...
                    referrer: canUseCreditInfo.saleManId
                  }
                });
              }, 500);
            ...
            this.cacheInfo.loanAmount = operateApplyRecord.data.data.tblLoanInfoEntity.loanAmount;

            this.cacheInfo.interestRate = operateApplyRecord.data.data.tblLoanInfoEntity.interestRate;

            this.cacheInfo.interestRateDesc = operateApplyRecord.data.data.tblLoanInfoEntity.interestRate
              ? operateApplyRecord.data.data.tblLoanInfoEntity.interestRate * 100 + '%'
              : '';

            this.cacheInfo.repayAccount = operateApplyRecord.data.data.tblBankCardAgreeInfoEntity.bankCardNo;

            this.loanTerm.forEach(item => {
              if (operateApplyRecord.data.data.tblLoanInfoEntity.loanTerm === item.value) {
                this.cacheInfo.loanTerm = operateApplyRecord.data.data.tblLoanInfoEntity.loanTerm;
                this.cacheInfo.loanTermDesc = item.label;
              }
            });

            this.loansUseType.forEach(item => {
              if (operateApplyRecord.data.data.tblLoanInfoEntity.loanPurpose === item.value) {
                this.cacheInfo.loanPurpose = operateApplyRecord.data.data.tblLoanInfoEntity.loanPurpose;
                this.cacheInfo.loanPurposeDesc = item.label;
              }
            });

            this.cacheInfo.loanNo = operateApplyRecord.data.data.tblLoanInfoEntity.loanNo;
          }
        }
      } catch (error) {
        beijingDialog({
        ...
        if (!res) {
          ...
          try {
            ...
            if (sendRes?.data?.success && sendRes.data.data) {
              this.borrowUserInfo.smsIdentificationCode = sendRes.data.data.referenceNo;
            }
      ...
      this.$refs.formDynamicUser.validate(['name', 'phone', 'smsCode']).then(async res => {
        if (!res) {
          ...
          try {
            let params = {
              applyUnionKey: this.applyUnionKey,
              loanNo: this.cacheInfo.loanNo,
              smsCode: this.borrowUserInfo.smsCode,
              referenceNo: this.borrowUserInfo.smsIdentificationCode
            };
            this.$toasting.show({ message: '上传中', forbidClick: true });
            let res = await beijingLoansVerifySmsCode(params);
            if (res?.data?.success) {
              setTimeout(() => {
                this.$router.replace({

 ...
 * @LastEditors: hsf
 * @LastEditTime: 2023-11-24 14:38:40
 * @Description:
-->
<template>
  <div class="loans-base-info-wrapper beijing-common">
          ...
          <van-form :label-width="80" ref="formDynamicBorrow">
            <div class="account-module">
              <van-field
                ...
                name="bankCard"
                label="放/还款账户"
                placeholder="请输入北京银行一、二类户"
                :readonly="bankCardList.length ? true : false"
                :rules="ruleValidate.bankCard"
                @input="changeBankCard"
              >
                <template #button>
                  <div
                    class="bank__arrow--select"
                    v-if="bankCardList.length ? true : false"
                    @click="openAccountSelect = true"
                  >
                    <img src="../../../static/image/home/right-icon.png" alt="" />
                  </div>
                </template>
              </van-field>
              <div class="operate-open-account">
                <div class="left-desc">
                  {{ bankCardList.length ? '请确认是否开立新的北京银行账户！' : '没有北京银行账户请先开户！' }}
                </div>
                <div class="tableTip" @click="goBeijingOpenAccount('common')">去开户</div>
              </div>
            </div>
            ...
            <div class="other-module">
              ...
              <van-field v-model="formItem.interestRateDesc" disabled name="interestRate" label="年化利率" readonly>
              </van-field>
              <van-field
    ...
    <!-- 开户提醒，暂时用不到, 在接口的验证如果不是北京银行的会提示 -->
    <van-popup :style="{ padding: '16px' }" v-model="isHasAccount" round class="my-loan-remind">
      <img src="../../../static/image/icon.png" alt="" class="icon" />
      <div class="title">尊敬的客户，您填写的银行</div>
      <div class="title">卡账号错误，请使用北京银行的一</div>
      <div class="title">类卡/二类户，或先进行开户再提款!</div>
      <div class="tip-remind">
        <span>开户成功后请点击</span>
        <span class="colorE0C">【返回】</span>
        <span>到本页面继续借款～</span>
      </div>
      <div class="account-operate__button">
        <van-button @click="goBeijingOpenAccount('dialog')" class="account-operate__button--left">知道了</van-button>
        <van-button @click="goBeijingOpenAccount('dialog')" class="account-operate__button--right ">去开户</van-button>
      </div>
    </van-popup>
...
export default {
  ...
  methods: {
    init() {
      try {
        this.formItem.productId = this.$route.query.productId || '1155';
    ...
    handleSubmit(name) {
      ...
      this.$refs.formDynamicBorrow.validate(['bankCard', 'borrowBalance']).then(async res => {
        if (!res) {
          try {
            let params = {
              applyLoanAmount: this.formItem.borrowBalance,
              applyUnionKey: this.formItem.applyUnionKey,
              bankCardNo: this.formItem.bankCard,
              customerName: this.userInfo.name,
              loanPurpose: this.formItem.loansPurpose,
              loanRate: this.formItem.interestRate,
              loanTerm: this.formItem.loansTimeLimit,
              phone: this.userInfo.phoneNum,
              repaymentType: '01'
            };
            this.$toasting.show({ message: '上传中', forbidClick: true });
            let res = await addBeijingSingleBorrowInfo(params);
            if (res?.data?.success) {
              sessionStorage.setItem('union_key_beijing', this.formItem.applyUnionKey);
    ...
    async getBankCardList() {
      try {
        let accountRes = await getBeijingRepayAccountList({ productId: '1155' });
    ...
    async getCreditNum() {
      try {
        this.$toasting.show({ message: '加载中', forbidClick: true });
        let creditList = await getUserCreditResult({ productId: this.formItem.productId || '1155' });

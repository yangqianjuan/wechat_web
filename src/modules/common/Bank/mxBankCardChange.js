...
export default {
  ...
  methods: {
    jdugeBankCardChange(params, isBack = false) {
      ...
      this.isBack = this.isBack || isBack;
      bankCardChange(params)
        .then((res) => {
          let data = res?.data?.data;
          this.bankcardChangeHandle(data);
        })
        .catch((err) => {
          this.errHandle(err);
        })
    ...
    bankcardChangeHandle(data) {
      ...
      if (type === '1001' || type === '1003' || type === '1004') {
        ...
        this.isBack ? this.$router.back(-1) : this.getBankCardList();
      } else if (type === '1005') {
    ...
    bindEaCard() {
      ...
      apllyEacount(bankCard)
        ...
        .catch((err) => {
          this.$notify.error({ msg: '电子账户申请失败！' });
        })
        .finally(() => {
          this.isBtnLoading = false;
        });
    ...
    errHandle(err) {
      this.isBtnLoading = false;
      this.itemIsLoading = false;
      this.$notify.error({ msg: err?.data?.msg || '操作失败！' });
    }
  }
};
